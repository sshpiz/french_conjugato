(function () {
  const MANIFEST_URL = '../french/tts/manifest.json';
  ...
  function packUrl(pack) {
    return `../french/tts/${pack.file}`;
  }

  function itemUrl(item) {
    return item && item.file ? `../french/tts/${item.file}` : null;
  }
  ...
  async function isPackCached(packId) {
    ...
    if (isClipLayout(manifest)) {
      ...
      for (const file of itemFiles) {
        const match = await cache.match(`../french/tts/${file}`);
      ...
      packPromises.set(packId, (async () => {
        try {
          if (isClipLayout(manifest)) {
            ...
            for (const file of itemFiles) {
              await ensureAssetCached(`../french/tts/${file}`, `file=${file}`);
            }
