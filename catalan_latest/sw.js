...
const CACHE_PREFIX = 'catalan-latest-app-cache-';
...
const TTS_PREFIX = appPath('../catalan/tts/');
...
async function serveWithSWR(request, url) {
  ...
  if (isVersionRequest) {
    try {
      ...
      if (response.ok) await cache.put(VERSION_PATH, response.clone());
      return response;
    } catch (e) {
  ...
  const refresh = fetch(request, (isNav || forceRefresh) ? { cache: 'no-store' } : {})
    .then(async response => {
      ...
      if (isNav) await cache.put(INDEX_PATH, response.clone());
    })
    .catch(() => {});
